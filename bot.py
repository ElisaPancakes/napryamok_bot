...
def poland(message):
    bot.send_message(message.chat.id, 'https://napryamok.org/ru#poland', reply_markup=rus_menu5)
...
def ua_needhelp_elsewhere_near(message):
    ...
    bot.send_message(message.chat.id, 'Виберіть країну, що межує з Україною', reply_markup=ua_menu5) 
...
def ua_poland(message):
    bot.send_message(message.chat.id, 'https://napryamok.org/#poland', reply_markup=ua_menu5)
